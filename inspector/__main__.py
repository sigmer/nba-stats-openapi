...
def main():
    ...
    doc['info'] = {
        'title': 'NBA Stats API',
        'description': ('OpenAPI documentation for the NBA Stats API '
                        'generated by sigmer/nba-stats-openapi'),
        'version': __version__
    }
...
def get_parameter_patterns(text):
    patterns = {}
    for error in text.split(';'):
        pat = r"^\s*The field (?P<param>((\w*)(\s\w+)*?)) must match" \
            r" the regular expression '(?P<pattern>(.*))'.?"
